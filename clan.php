 ...
 if(isset($_POST['join']))
{
    ...
    // Remove the check for the number of clans a user can join
    if(!$isIn && $userID != 0)
    {
...
<!DOCTYPE html>
        ...
        <div style="margin:10px; vertical-align:top;">
          ...
          <div style="height:240px;">
            <div style="width:200px;float:left;">
              <img style="width:200px;height:200px;" src="/images/clans/<?php echo $thumbnail; ?>.png">
 ...
 <div id="wall">
    <?php
    $wallSQL = "SELECT * FROM `clans_walls` WHERE `group_id`='$clanID' AND (`type`='normal' OR `type`='pinned') ORDER BY `time` DESC LIMIT 10;";
    $wallResult = $conn->query($wallSQL);
    while ($wallRow = $wallResult->fetch_assoc()) {
        $ownerID = $wallRow['owner_id'];
        $ownerSQL = "SELECT * FROM `beta_users` WHERE `id`='$ownerID';";
        $ownerResult = $conn->query($ownerSQL);
        $ownerRow = $ownerResult->fetch_assoc();
        $username = $ownerRow['username'];

        // Make the username clickable
        $profileLink = '<a style="color:#555;" href="/user?id=' . $ownerID . '">' . $username . '</a>';

        // Output the wall post
        echo '<div>';
        echo '<p>' . $profileLink . ' said: ' . htmlentities($wallRow['post']) . '</p>';
        echo '<span>' . $wallRow['time'] . '</span>';
        
        // Output the avatar and make it clickable
        echo '<a href="/user?id=' . $ownerID . '"><img src="/path/to/avatars/' . $ownerID . '.png" alt="' . $username . '"></a>';
...
<style>
  img {
    max-width: 100px; /* You can adjust this value to your desired maximum width */
    max-height: 100px; /* You can adjust this value to your desired maximum height */
    width: auto;
    height: auto;
  }
</style>
