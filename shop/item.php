...
<!DOCTYPE html>
  ...
  <meta property="og:title" content="<?php echo $shopRow['name']; ?> - talka-boutit item." />
      ...
      echo '</h3>';
        ...
        <span style="display:inline-block; float:left; width:640px;">
          ...
          if(isset($_GET['render']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #000 solid;width:340px;height:340px;" src="/customize/rendering/shop_render?id='.$shopRow['id'].'"></iframe>';
          }
          else {
...
<?php } ?>
        ...
        while($commentRow = mysqli_fetch_assoc($comments)) {
              ...
              echo'<div id="subsect" style="overflow:auto;"><span style="float:left;"><a href="/user?id='.$commentUserID.'" style="color:#333;"><img style="width:55px;" src="/images/avatars/'.$commentUserID.'.png?c='.rand().'"><br>'.$userData['username'].'</a></span><span><span style="font-size:12px;">'.$commentRow['time'].'</span><br>'. htmlentities($commentRow['comment']).'</span>';
              if($loggedIn && $power >= 5) {
